...
class DQN(QLearnCont):
    ...
    def save_learner(self,idx):
    	json_string = self.learner.to_json()  
    	open('nn/my_model_architecture.json','w').write(json_string)  
    	self.learner.save_weights('nn/my_model_weights_%d.h5'%idx,overwrite=True)
  
    def load_learner(self,idx):
    	self.learner = model_from_json(open('nn/my_model_architecture.json').read())
    	self.learner.load_weights('nn/my_model_weights_%d.h5'%idx)  
    	self.epsilon=0.0    
        ...
        if self.learn_type == 'nn':
            ...
            for i in range(len(shape)):
                if i == 0:
                    ...
                    nb_input = shape[i -1]
                nb_output = shape[i]
                model.add(Dense(input_dim=nb_input,output_dim=nb_output,init="he_normal",
                            activation = "tanh"))
                #model.add(Dropout(.5))
            model.add(Dense(input_dim=shape[-1],output_dim=1,init = "he_normal",
                            activation = "linear"))
    ...
    def qUpdate(self,state,action,reward,next_state):
        ...
        for i,row in enumerate(sample):  #i is index, row is value, totaly 100 rows repeat
            ...
            #get feature vector
            d[i,:] = self.feat_funct(state,action)
            #get target
...
#ipdTft(length=80000,gamma=0.9,epsilon=0.2,alpha = .8)
